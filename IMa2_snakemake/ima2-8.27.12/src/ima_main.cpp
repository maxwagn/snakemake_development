...
static void savegenealogyinfo (void);
...
scan_commandline (int argc, char *argv[])
{
  ...
  {
    printf ("IMa2 Program - copyright 2012 by Jody Hey, Rasmus Nielsen, Sang Chul Choi, Vitor Sousa and Janeen Pisciotta\n");
...
print_outputfile_info_string (void)
{
  ...
  *fpstri = 0;
  SP "IMa version 2.0 - Isolation with Migration Analysis  -  Jody Hey, Rasmus Nielsen, Sang Chul Choi, Vitor Sousa,  Janeen Pisciotta 2012 \n");
  SP "Release date: %s\n\n",RELEASE_DATE);
...
void savegenealogyinfo (void)        // use floats to save space
{
  int i;
...
void intervaloutput (FILE * outto)
{
      ...
      || outto != stdout)
  {
    ...
    if (numchains > 1)
      printchaininfo (outto, heatmode, hval1, hval2);
    if (genealogiessaved > 0)
    {
...
void check_to_record (void)
{
  ...
  {
    savegenealogyinfo ();            // record values associated with genealogies
    if (jheyoptions[WRITEMIGRATIONNAME])
      record_migration_names();
    if (assignmentoptions[POPULATIONASSIGNMENT] == 1)
    {
