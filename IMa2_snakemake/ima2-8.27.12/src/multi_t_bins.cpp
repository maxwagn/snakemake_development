...
setup_multi_t_arrays ()
{
  ...
  for (i = 0; i < numt; i++)
  {
    if (C[0]->tvals[i] > T[i].pr.max)
      return;
    if (i == 0)
      tvalarraypos[i] = (int) (NUMTARRAYBINS * ((C[0]->tvals[i] - T[i].pr.min) / (tpriormax  - T[i].pr.min)));
    else
      tvalarraypos[i] = (int) (NUMTARRAYBINS * ((C[0]->tvals[i] - T[i].pr.min) / (tpriormax  - T[i].pr.min))) - tvalarraypos[i - 1];
  }
  if (numt == 2)
  {
